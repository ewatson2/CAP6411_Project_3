...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, ori_img, class_idxs):
        self.height, self.width = ori_img.shape[:2]
        # generate detections
        features = self._get_features(bbox_xywh, ori_img)
        bbox_tlwh = self._xywh_to_tlwh(bbox_xywh)
        detections = [Detection(bbox_tlwh[i], conf, features[i], class_idxs[i]) for i,conf in enumerate(confidences) if conf>self.min_confidence]
        ...
        for track in self.tracker.tracks:
            ...
            outputs.append(np.array([x1,y1,x2,y2,track_id,class_idx,confidence], dtype=np.float32))
        if len(outputs) > 0:
            outputs = np.stack(outputs,axis=0)
        return outputs

    def update_ages(self):
        self.tracker.update_ages()
